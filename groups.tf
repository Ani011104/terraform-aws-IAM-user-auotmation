...
resource "aws_iam_group_membership" "education_members" {
  ...
  users = [
    for user in aws_iam_user.iam_users : user.name if user.tags.Department == "Education"
  ]
}
...
resource "aws_iam_group_membership" "managers_members" {
  ...
  users = [
    for user in aws_iam_user.iam_users : user.name if contains(keys(user.tags), "JobTitle") && can(regex("Manager|CEO", user.tags.JobTitle))
  ]
}
...
resource "aws_iam_group_membership" "engineers_members" {
  ...
  users = [
    for user in aws_iam_user.iam_users : user.name if user.tags.Department == "Engineering" # Note: No users match this in the current CSV
  ]
}
